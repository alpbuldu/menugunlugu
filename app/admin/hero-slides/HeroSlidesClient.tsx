...
function ContentPicker({
  ...
  const [filtered, setFiltered] = useState<PickerItem[]>([]);
  const [search, setSearch] = useState("");
  const [loading, setLoading] = useState(true);

  useEffect(() => {
    const supabase = createClient();
    const query = mode === "recipe"
      ? supabase
          .from("recipes")
    ...
    query.then(({ data }) => {
      const list = (data ?? []).filter((r: PickerItem) => r.image_url) as PickerItem[];
      setItems(list);
      setFiltered(list);
      setLoading(false);
    });
  ...
  useEffect(() => {
    ...
    setFiltered(q ? items.filter(r => r.title.toLowerCase().includes(q)) : items);
  }, [search, items]);

  const heading = mode === "recipe" ? "Tariften Seç" : "Blog Yazısından Seç";
  const hrefPrefix = mode === "recipe" ? "/tarifler/" : "/blog/";

  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/60" onClick={onClose}>
      <div className="bg-white rounded-2xl shadow-2xl w-full max-w-2xl max-h-[80vh] flex flex-col" onClick={e => e.stopPropagation()}>
        <div className="flex items-center justify-between p-4 border-b border-warm-100">
          <h3 className="font-semibold text-warm-800">{heading}</h3>
            ...
            <div className="grid grid-cols-3 sm:grid-cols-4 gap-2">
              {filtered.map(r => (
                <button
                  ...
                  type="button"
                  onClick={() => {
                    onSelect({ imageUrl: r.image_url, title: r.title, href: `${hrefPrefix}${r.slug}` });
                    onClose();
                  }}
...
const TINT_PRESETS = [
  ...
  { label: "Mor %40",     value: "bg-[#3D1F5C]/40" },
...
// DB'de \n literal olarak saklandığında gerçek satır sonu'na çevir
function parseTitle(t: string | null): string {
  return (t ?? "").replace(/\\n/g, "\n");
}
...
function SlideForm({
  ...
  const [title,     setTitle]     = useState(parseTitle(initial?.title ?? ""));
  ...
  const [picker,     setPicker]    = useState<"recipe" | "blog" | null>(null);
    ...
    <div className="bg-white rounded-2xl border border-warm-200 shadow-sm p-6 mb-4">
      ...
      {dynamicInfo && (
        <div className="mb-4 flex items-start gap-2 bg-blue-50 border border-blue-100 rounded-xl px-3 py-2.5">
          ...
          <p className="text-xs text-blue-700">{dynamicInfo}. Buraya değer girilirse otomatik veriyi geçersiz kılar.</p>
        </div>
      )}
      ...
      <form onSubmit={handleSubmit} className="space-y-4">
        ...
        <div>
          ...
          <textarea value={title} onChange={e => setTitle(e.target.value)} rows={2}
            placeholder={dynamicInfo ? "Boş bırakılırsa dinamik veri kullanılır" : "Slide başlığı — Enter ile satır atla"}
          ...
          <p className="text-[11px] text-warm-400 mt-1">Satır atlamak için Enter'a bas</p>
        </div>
        ...
        <div>
          ...
          {imageUrl && (
            <div className="mb-2 rounded-xl overflow-hidden border border-warm-100 relative">
          ...
          <div className="flex items-center gap-2 flex-wrap">
            ...
            <button type="button" onClick={() => setPicker("recipe")}
              className="px-3 py-2 border border-brand-200 rounded-xl text-sm text-brand-600 hover:bg-brand-50 transition-colors">
              Tariften Seç
            </button>
            <button type="button" onClick={() => setPicker("blog")}
              className="px-3 py-2 border border-brand-200 rounded-xl text-sm text-brand-600 hover:bg-brand-50 transition-colors">
              Blogtan Seç
            </button>
            {imageUrl && <span className="text-xs text-green-600 font-medium">✓ Seçildi</span>}
          </div>
          <p className="text-[11px] text-warm-400 mt-1">Tariften veya blogtan seçince başlık ve link de otomatik doldurulur.</p>
...
export default function HeroSlidesClient({ initialSlides }: Props) {
  ...
  async function moveSlide(slide: HeroSlide, direction: "up" | "down") {
    ...
    const [resA, resB] = await Promise.all([
      fetch("/api/admin/hero-slides", { method: "PATCH", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ id: slide.id, sort_order: other.sort_order }) }),
      fetch("/api/admin/hero-slides", { method: "PATCH", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ id: other.id, sort_order: slide.sort_order }) }),
    ]);
    if (resA.ok && resB.ok) {
      const [updA, updB] = await Promise.all([resA.json(), resB.json()]);
      setSlides(prev => prev.map(s => s.id === updA.id ? updA : s.id === updB.id ? updB : s).sort((a, b) => a.sort_order - b.sort_order));
    }
  }
  ...
  return (
    <div>
      ...
      <div className="mb-5 bg-amber-50 border border-amber-200 rounded-2xl px-4 py-3 text-xs text-amber-800">
        <strong>Not:</strong> Sistem slide'ları (günün menüsü, son tarif, blog) bazı alanlarını otomatik doldurur. Sırayı ↑↓ butonlarıyla değiştirebilirsin.
      </div>
                  ...
                  <div className="flex items-stretch">
                    ...
                    <div className="w-20 sm:w-28 flex-shrink-0 relative bg-warm-100">
                      {slide.image_url ? (
                        ...
                        <div className="w-full h-full flex items-center justify-center text-2xl" style={{ minHeight: 72 }}>🖼️</div>
                      )}
                      {slide.slide_key && (
                        <span className="absolute bottom-1 left-1 bg-blue-600 text-white text-[9px] font-bold px-1.5 py-0.5 rounded-md uppercase">sistem</span>
                      )}
                    </div>
                    ...
                    <div className="flex-1 min-w-0 p-3">
                      <div className="flex items-start gap-2 flex-wrap">
                        <span className="inline-block bg-brand-100 text-brand-700 text-[10px] font-bold px-2 py-0.5 rounded-full uppercase">{slide.badge}</span>
                        {!slide.is_active && (
                          <span className="inline-block bg-warm-100 text-warm-500 text-[10px] font-bold px-2 py-0.5 rounded-full uppercase">Pasif</span>
                        )}
                      </div>
                      <p className="font-semibold text-warm-800 text-sm mt-1 line-clamp-1">
                        {slide.title
                          ? parseTitle(slide.title)
                          : <span className="italic text-warm-400">Dinamik başlık</span>
                        }
                      </p>
                      {slide.cta_href
                    ...
                    <div className="flex flex-col items-end justify-between gap-1 p-3 flex-shrink-0">
                      <div className="flex flex-col gap-0.5">
                        <button onClick={() => moveSlide(slide, "up")} disabled={idx === 0}
                          className="w-7 h-7 flex items-center justify-center text-warm-400 hover:text-warm-700 disabled:opacity-20 border border-warm-100 rounded-lg text-xs transition-colors">↑</button>
                        <button onClick={() => moveSlide(slide, "down")} disabled={idx === slides.length - 1}
                          className="w-7 h-7 flex items-center justify-center text-warm-400 hover:text-warm-700 disabled:opacity-20 border border-warm-100 rounded-lg text-xs transition-colors">↓</button>
                      </div>
                      <div className="flex gap-1.5 flex-wrap justify-end">
                        <button onClick={() => toggleActive(slide)}
                          className={`px-2.5 py-1 rounded-lg text-xs font-medium transition-colors border ${slide.is_active ? "bg-green-50 border-green-200 text-green-700 hover:bg-green-100" : "bg-warm-50 border-warm-200 text-warm-500 hover:bg-warm-100"}`}>
