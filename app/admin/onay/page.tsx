...
export const metadata: Metadata = { title: "Tarif Onayı" };
...
export default async function OnayPage() {
  ...
  const recipes = (data ?? []) as PendingRecipe[];

  return (
    <div>
      <div className="mb-8 flex items-center justify-between">
        <div>
          <h1 className="text-2xl font-bold text-warm-900">Tarif Onayı</h1>
          <p className="text-warm-500 text-sm mt-1">
            Üyelerden gelen ve inceleme bekleyen tarifler
          </p>
        </div>
        <span className="px-3 py-1 rounded-full bg-yellow-100 text-yellow-700 text-sm font-medium">
          {recipes.length} bekliyor
        </span>
      </div>
      ...
      {recipes.length === 0 ? (
        <div className="bg-white rounded-2xl border border-warm-200 p-12 text-center text-warm-400">
          ...
          <p className="font-medium">İncelenecek tarif yok</p>
          <p className="text-sm mt-1">Tüm tarifler incelendi.</p>
        </div>
      ) : (
          ...
          {recipes.map((recipe) => (
            <div key={recipe.id} className="bg-white rounded-2xl border border-warm-200 overflow-hidden">
              ...
              <div className="border-t border-warm-100 px-5 py-3 bg-warm-50 flex items-center justify-end gap-3">
                <ApprovalActions recipeId={recipe.id} />
              </div>
            </div>
          ))}
        </div>
